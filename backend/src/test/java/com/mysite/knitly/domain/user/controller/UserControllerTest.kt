...
class UserControllerTest {
    ...
    fun `logout Success WithAccessToken`() {
        ...
        // then
        assertThat(result.statusCode.value()).isEqualTo(200)
        assertThat(result.body).isEqualTo("[Auth] [UserController] 로그아웃되었습니다.")

        // 검증: authService.logout()이 2번 호출되는지 확인
    ...
    fun `logout Success WithoutAuthHeader`() {
        // given
        doNothing().`when`(cookieUtil).deleteCookie(any(), eq("refreshToken"))
        ...
        // Authorization 헤더가 없으면 JWT 파싱 시도하지 않음
        verify(jwtProvider, never()).getUserIdFromToken(anyString())

        // authService.logout()도 호출되지 않음 (정상 동작)
    ...
    fun `logout TokenParsingFails StillDeletesCookie`() {
        ...
        // 토큰 파싱 실패 시 authService.logout()은 호출되지 않음 (정상)
        verify(authService, never()).logout(any())
    }
    ...
    fun `deleteAccount Fail Unauthorized`() {
        ...
        // then
        assertThat(result.statusCode.value()).isEqualTo(401)
        assertThat(result.body).isEqualTo("[Auth] [UserController] 인증이 필요합니다.")
